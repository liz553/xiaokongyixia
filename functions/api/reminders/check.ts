...
export const onRequestGet = async (context: any) => {
  ...
  try {
    ...
    // Find pending pushes where target_time <= nowUnix
    // Allow 2 mins margin for late cron execution
    ...
    for (const check of pendingMissChecks.results) {
      ...
      // Check if user created any logs since timeSentIso
    ...
    return withCors(successResponse({ sent: sentCount }), request, env);
  } catch (e: any) {
